...
<section class="wrapper style special">
	...
	$statement = $pdo->prepare("SELECT * FROM MSkywars_stats WHERE Rank = ?");
...
while($row = $statement->fetch()) {
	  ...
	  ?>
                                        ...
                                        <p>Kisten geöffnet: <?php echo $beds;?></p>
	...
	$statement = $pdo->prepare("SELECT * FROM MSkywars_stats WHERE UUID = ?");
...
while($row = $statement->fetch()) {
	  ...
	  ?>
		...
		?><h4>Der Spieler <?php echo $_POST['username'];?> wurde nicht gefunden, er hat anscheinend noch nie Skywars gespielt. <p>
		<p>mh schade..</h4><?php
	}
